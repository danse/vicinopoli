...
| `composer` | `composer-*`, `composer-change-pseudonym` | Compose/publish; message-type chips (`composer-type-*`) show only the selected input — textarea for text, single-line caption (`composer-caption`) for photo/voice; "posting as" link -> `pseudonym` |
